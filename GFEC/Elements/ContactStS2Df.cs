...
namespace GFEC
{
    class ContactStS2Df : IElement
    {
        ...
        public ContactStS2Df(IElementProperties properties, Dictionary<int, INode> nodes)
        {
            ...
            //for (int i = 1; i <= Properties.IntegrationPoints; i++)
            //{
            //    ElementFreedomSignature[i] = new bool[] { true, true, false, false, false, false };
        ...
        private double[] NodalXInitial()
        {
            ...
            {
                int count = (i - 1) * 2;
                x[count] = Nodes[i].XCoordinate;
                x[count + 1] = Nodes[i].YCoordinate;
            }
            return x;
        }
        private double[] NodalXUpdatedForPrevLoadSteps(double [] PreviousDisplacement)
        {
            int amountOfNodes = Properties.SlaveSegmentPolynomialDegree + Properties.MasterSegmentPolynomialDegree + 2;
            double[] x = new double[2 * amountOfNodes];
            for (int i = 1; i <= amountOfNodes; i++)
            {
                int count = (i - 1) * 2;
                x[count] = Nodes[i].XCoordinate + PreviousDisplacement[count];
                x[count + 1] = Nodes[i].YCoordinate + PreviousDisplacement[count + 1];
            }
            return x;
        }
        ...
        private Tuple<double[], double, double[], double[], double> MasterSegmentGeometry(double[,] daMatrix, double[,] da2Matrix)
        {
            ...
            if (Properties.MasterSegmentPolynomialDegree == 1)
            {
                ...
                vector2[0] = Ym2 - Ym1;
                vector2[1] = Xm2 - Xm1;
                tangentVector = VectorOperations.VectorScalarProductNew(vector2, 1.0 / VectorOperations.VectorNorm2(vector2));
            }
            else
            {
        ...
        private double[,] CalculateTangentialStiffnessPartForSticking(double[,] A, double[,] dA, double Tangtr, double[] normalVector, double[] tangentVector, double detm, double m11, double h11)
        {
            double coef2 = Tangtr * m11;
            double coef3 = Tangtr * h11 * Math.Pow(detm,0.5);
            double[,] tangentialPart1;
            double[,] tangentialPart2;
            double[,] tangentialPart3;
            double[,] FullTangentialPart;


            double[,] t_x_t = VectorOperations.VectorVectorTensorProduct(tangentVector, tangentVector);
            double[,] t_x_n = VectorOperations.VectorVectorTensorProduct(tangentVector, normalVector);
            double[,] n_x_t = VectorOperations.VectorVectorTensorProduct(normalVector, tangentVector);

            double[,] Matrix1 = MatrixOperations.MatrixProduct(
                                                                    MatrixOperations.Transpose(A),
                                                                    MatrixOperations.MatrixProduct(t_x_t, A)
                                                                    );
            tangentialPart1 = MatrixOperations.ScalarMatrixProductNew(
                                                                        TangentialPenaltyFactor,
                                                                        Matrix1
                                                                        );
            ...
            tangentialPart2 = MatrixOperations.ScalarMatrixProductNew(
                                                                        coef2,
                                                                        Matrix2
                                                                        );
            double[,] Matrix3 = MatrixOperations.MatrixProduct(
                                            MatrixOperations.Transpose(A),
                                            MatrixOperations.MatrixProduct(MatrixOperations.MatrixAddition(t_x_n, n_x_t), A)
                                            );
            tangentialPart3 = MatrixOperations.ScalarMatrixProductNew(
                                                                        coef3,
                                                                        Matrix3
                                                                        );
            FullTangentialPart = MatrixOperations.MatrixAddition(MatrixOperations.MatrixAddition(MatrixOperations.ScalarMatrixProductNew(-1.0, tangentialPart1), MatrixOperations.MatrixAddition(tangentialPart2, MatrixOperations.Transpose(tangentialPart2))), MatrixOperations.Transpose(tangentialPart3));
            return FullTangentialPart;
        }
        private double[,] CalculateTangentialStiffnessPartForSliding(double[,] A, double[,] dA, double Tangtr, double[] normalVector, double[] tangentVector, double detm, double m11, double h11, double ksi3)
        {
            double coef1 = Properties.SlidingCoefficient * PenaltyFactor * (Tangtr / Math.Abs(Tangtr));
            double coef2 = Properties.SlidingCoefficient * PenaltyFactor * (Tangtr / Math.Abs(Tangtr)) * Math.Abs(ksi3) * Math.Pow(m11,0.5);
            double coef3 = Properties.SlidingCoefficient * PenaltyFactor * (Tangtr / Math.Abs(Tangtr)) * Math.Abs(ksi3) * h11 * detm;
            double[,] tangentialPart1;
            double[,] tangentialPart2;
            double[,] tangentialPart3;
            double[,] FullTangentialPart;
            ...
            double[,] Matrix1 = MatrixOperations.MatrixProduct(
                                                                    MatrixOperations.Transpose(A),
                                                                    MatrixOperations.MatrixProduct(t_x_n, A)
                                                                    );
            tangentialPart1 = MatrixOperations.ScalarMatrixProductNew(
                                                                        coef1,
                                                                        Matrix1
        ...
        public double[,] CreateGlobalStiffnessMatrix()
        {
            ...
            Dictionary<int, double> initialStickingPoints = Properties.AllIntegrationPointsStickingPoints;
            Dictionary<int, double> initialTangentTractions = Properties.AllIntegrationPointsTangentialTractions;
            for (int i = 0; i < Properties.IntegrationPoints; i++)
            {
                ...
                if (Math.Abs(ksi1) <= 1.05)
                {
                    ...
                    double detm = VectorOperations.VectorDotProduct(dRho, dRho);
                    ...
                    double[] t = masterSurfaceCharacteristics.Item4;
                    ...
                    if (ksi3 <= 0)
                    {
                        ...
                        double[,] StifnessMatrixNormalPart = MatrixOperations.ScalarMatrixProductNew(scalar, MatrixOperations.MatrixAddition(MatrixOperations.MatrixAddition(mainPart, rotationalPart),
                            curvaturePart));
                        globalStifnessMatrix = MatrixOperations.MatrixAddition(globalStifnessMatrix, StifnessMatrixNormalPart);
                        double DeltaKsi1 = ksi1 - initialStickingPoints[i];
                        double tangentialTraction = initialTangentTractions[i] - detm * TangentialPenaltyFactor * DeltaKsi1;
                        double phiTr = Math.Pow(tangentialTraction * tangentialTraction * m11, 0.5) - Properties.StickingCoefficient * PenaltyFactor * Math.Abs(ksi3);
                        if (phiTr <= 0)
                        {
                            //Sticking
                            double T1 = tangentialTraction;
                            double[,] StifnessMatrixTangentialPart = MatrixOperations.ScalarMatrixProductNew(scalar, CalculateTangentialStiffnessPartForSticking(aMatrix, daMatrix, T1, n, t, detm, m11, h11));
                            globalStifnessMatrix = MatrixOperations.MatrixAddition(globalStifnessMatrix, StifnessMatrixTangentialPart);

                        }
                        else
                        {
                            //Sliding
                            //double T1 = (tangentialTraction / Math.Abs(tangentialTraction)) * Properties.SlidingCoefficient * PenaltyFactor * Math.Abs(ksi3) * Math.Pow(detm, 0.5);
                            double[,] StifnessMatrixTangentialPart = MatrixOperations.ScalarMatrixProductNew(scalar, CalculateTangentialStiffnessPartForSliding(aMatrix, daMatrix, tangentialTraction, n, t, detm, m11, h11, ksi3));
                            globalStifnessMatrix = MatrixOperations.MatrixAddition(globalStifnessMatrix, StifnessMatrixTangentialPart);
                        }
                    }
                }
                ...
                if (Math.Abs(ksi1) <= 1.05)
                {
                    ...
                    if (ksi3 <= 0)
                    {
                        ...
                        if (phiTr <= 0)
                        {
                            ...
                            double[] internalTangentialForcesVector = VectorOperations.VectorScalarProductNew(VectorOperations.VectorScalarProductNew(AT_t, T1 * Math.Pow(m11, 0.5)), scalar);
                            internalGlobalForcesVector = VectorOperations.VectorVectorAddition(internalGlobalForcesVector, internalTangentialForcesVector);
        ...
        private double ProjectInitialStickingpoint(double ksi1Initial, double ksi2)
        {
            if (Properties.MasterSegmentPolynomialDegree == 1)
            {
                double[,] aMatrix = CalculatePositionMatrix(ksi1Initial, ksi2).Item1;
                ...
                double[] slavePositionVector = VectorOperations.MatrixVectorProduct(slaveNMatrix, x);
                double xM1 = xInitial[0];
                double yM1 = xInitial[1];
                double xM2 = xInitial[2];
                double yM2 = xInitial[3];
                double xS = slavePositionVector[0];
                double yS = slavePositionVector[1];
                double ksi = (2 * (xS * (xM2 - xM1) + yS * (yM2 - yM1)) - Math.Pow(xM2, 2) - Math.Pow(yM2, 2) + Math.Pow(xM1, 2) + Math.Pow(yM1, 2)) / (Math.Pow(xM2 - xM1, 2) + Math.Pow(yM2 - yM1, 2));
                return ksi;
            ...
            else
            {
                ...
                {
                    Tuple<double[,], double[,], double[,], double[,], double[,]> aMatrices = CalculatePositionMatrix(ksi, ksi2);
                    double[] masterSlaveRelativeVector = VectorOperations.MatrixVectorProduct(aMatrices.Item1, xInitial);
                    double[] surfaceVector = VectorOperations.VectorScalarProduct(VectorOperations.MatrixVectorProduct(aMatrices.Item2, xInitial), -1);
                    double[] surfaceVectorDerivative = VectorOperations.VectorScalarProduct(VectorOperations.MatrixVectorProduct(aMatrices.Item3, xInitial), -1);
        ...
        {
            //double[] initialStickingPoints = new double[Properties.IntegrationPoints];
            for (int i = 0; i < Properties.IntegrationPoints; i++)
            {
                double ksi2 = GaussPoints(i).Item1;
                double ksi11 = ProjectInitialStickingpoint(0.0, ksi2);
                AllIntegrationPointsStickingPoints.Add(i, ksi11);
                AllIntegrationPointsTangentialTractions.Add(i, 0.0);
            }
        ...
        public void UpdateTangentialProperties()
        {
            ...
            {
                double ksi2 = GaussPoints(i).Item1;
                double ksi11 = Project(0.0, ksi2);
                double DeltaKsi1 = ksi11 - Properties.AllIntegrationPointsStickingPoints[i];
                Tuple<double[,], double[,], double[,], double[,], double[,]> positionMatrices = CalculatePositionMatrix(ksi11, ksi2);
                double[,] aMatrix = positionMatrices.Item1;
                double[,] daMatrix = positionMatrices.Item2;
                double[,] da2Matrix = positionMatrices.Item3;
                Tuple<double[], double, double[], double[], double> masterSurfaceCharacteristics = MasterSegmentGeometry(daMatrix, da2Matrix);
                double[] dRho = masterSurfaceCharacteristics.Item1;
                double detm = VectorOperations.VectorDotProduct(dRho, dRho);
                double tangentialTraction = Properties.AllIntegrationPointsTangentialTractions[i] - detm * TangentialPenaltyFactor * DeltaKsi1;
                Properties.AllIntegrationPointsStickingPoints[i] = ksi11;
                Properties.AllIntegrationPointsTangentialTractions[i] = tangentialTraction;
            }
        }
    }
}
