...
namespace GFEC
{
    public static class CNTsInParallelFinalSensitivityAnalysis2
    {
        private const int offsetNodes = 2; /*73;*///65;/*55;*//*41;*//*29;*///17;//2
        private const int totalNodes = 1596;//2254; 648; 318; 116;
        private const int addedNodesrodElements = 2;
        private const int totalElements = 1320;//1920; 480; 208; 56;
        private const int nodesInXCoor = 133;//161
        private const int nodesInYCoor = 6;//7
        private const int totalContactElements = nodesInXCoor - offsetNodes + 1;//20;//8;
        private const double scaleFactor = 1.0;
        private const double xIntervals = 0.226145;
        private const double xIntervals0 = 0.375;
        private const double yIntervals = 0.246;
        private const double offset = (offsetNodes - 1) * xIntervals0;//8.1;//9.3;
        ...
        //const double externalStructuralLoad = -20.0 * xIntervals * thickness * (double)nodesInXCoor / (double)totalContactElements;
        const double externalStructuralLoad = -20.0 * xIntervals * thickness * 1.0125;
        const double T0 = 100.0;
        const double solidThermalCond = 3300;
        static double externalHeatLoad = -2 * T0 * (solidThermalCond / (6 * xIntervals0 * yIntervals)) * ((Math.Pow(xIntervals0, 2) - 2 * Math.Pow(yIntervals, 2)) - (Math.Pow(xIntervals0, 2) + Math.Pow(yIntervals, 2)));        //const double externalStructuralLoad = -5 * 100000000.0 * 1e-18 * 0.3;
        ...
        private static Dictionary<int, INode> CreateNodes()
        {
            ...
            for (int j = 0; j < nodesInYCoor; j++)
            {
                for (int i = 0; i < nodesInXCoor; i++)
                {
                    ...
                    {
                        nodes[k] = new Node(i * xIntervals0 * scaleFactor, j * yIntervals * scaleFactor);
                    }
                    else
                    {
                        nodes[k] = new Node(xIntervals0 + (i - 1) * xIntervals * scaleFactor, j * yIntervals * scaleFactor);
                    }
                    k += 1;
                }
            }
            ...
            for (int j = 0; j < nodesInYCoor; j++)
            {
                for (int i = 0; i < nodesInXCoor; i++)
                {
                    if(i == nodesInXCoor - 1)
                    {
                        nodes[k] = new Node(xIntervals0 + (i - 1) * xIntervals * scaleFactor + offset, j * yIntervals * scaleFactor - ((nodesInYCoor - 1) * yIntervals + gap));
                    }
                    else
                    {
            ...
            //}
            nodes[totalNodes + 1] = new Node(offset, yIntervals * (nodesInYCoor - 1) + 0.41);
            nodes[totalNodes + 2] = new Node(xIntervals * scaleFactor * (nodesInXCoor - 2) + xIntervals0, yIntervals * (nodesInYCoor - 1) + 0.41);
        ...
        public static Results RunStaticExample()
        {
            ...
            Dictionary<int, double[]> allStepsHeatFluxes = new Dictionary<int, double[]>();
            Dictionary<int, double[]> allStepsTemperatureJumps = new Dictionary<int, double[]>();
            ...
            for (int k = 1; k <= allStepsSolutions.Count; k++)
            {
                ...
                List<double> elementsInternalHeatFluxVectors = new List<double>();
                List<double> contactElementsTemperatureJumps = new List<double>();
                for (int j = totalElements + 1; j <= totalElements + totalContactElements; j++)
                {
                    elementsInternalHeatFluxVectors.Add(
                        elementsAssembly2.ElementsAssembly[j].CreateInternalGlobalForcesVector()[1]);
                    contactElementsTemperatureJumps.Add(elementsAssembly2.ElementsAssembly[j].GetStressVector()[0][0]);
                }
                allStepsHeatFluxes[k] = elementsInternalHeatFluxVectors.ToArray();
                allStepsTemperatureJumps[k] = contactElementsTemperatureJumps.ToArray();
                ...
                string name1 = "Q" + k.ToString() + ".dat";
                string name2 = "TD" + k.ToString() + ".dat";
                VectorOperations.PrintVectorToFile(allStepsHeatFluxes[k], @"C:\Users\Public\Documents\" + name1);
                VectorOperations.PrintVectorToFile(allStepsTemperatureJumps[k], @"C:\Users\Public\Documents\" + name2);
            }
